...
| # | 功能 | 说明 |
|---|------|------|
| 1 | 单端口检测 | 同时检测 TCP/UDP，显示占用进程名、PID、文件完整路径，附带常见端口说明（如 3306=MySQL）；支持按 K 一键结束占用进程释放端口，主菜单可直接输入端口号快速检测 |
...
| `1` ~ `9` | 主菜单选择功能 |
| `0` | 退出程序 |
| 直接输入端口号 | 主菜单处输入 2 位以上数字（如 `8001`）直接检测该端口，无需先进菜单 [1] |
| `K` | 单端口检测结果后，一键结束占用进程释放端口 |
